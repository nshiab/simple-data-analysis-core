...
export default class SimpleTable extends Simple {
   ...
   * added as `umapX` and `umapY`. Neighbor search is selected automatically.
   *
   * `neighbors` controls how many nearby points influence the layout: smaller
   * values emphasize local groups, while larger values show broader structure.
   * `metric` defines similarity: Euclidean compares distance, while cosine
   * compares direction. `minDistance` controls how tightly points can group
   * together in the projection; smaller values allow tighter groups.
   *
   * `epochs` is the number of passes used to refine the coordinates. More passes
   * allow more refinement but take longer. `seed` is an integer that controls
   * the random choices during optimization. Keep the seed and input row order
   * fixed when comparing repeated runs.
   *

...
produce separate results and `pathId` values even when their cycles use the same
connections in rotated order. Static searches do not require the starting node
to be the smallest node. With `direction: "both"`, reverse sequences are
deduplicated within each start by retaining the lexicographically smaller
edge-key sequence. Cycles are numbered by starting node key, then edge-key
sequence. Numeric keys use native numeric order; string keys use binary bytes,
independently of collations. Input row order and the order of requested starts
do not affect numbering. Parallel connections with distinct IDs remain distinct.

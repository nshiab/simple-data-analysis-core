...
and finite and its effective end is not before its effective start. Before any
graph computation, the six chronological methods run a separate native DuckDB
existence check and reject the whole supplied table if any event is invalid. The
...
`shortestPath()` and `paths()` return `pathId`, `step`, `weight`, and `total`
first. `findCycles()` returns `start`, `pathId`, `step`, `weight`, and `total`
first. These generated columns are followed by every original input column in
its original order. Original names, values, and types are preserved, including
the columns selected as connection ID, source, target, weight, and timestamps.
There are no additional standardized `edgeId`, `source`, or `target` columns.
The unique, non-null connection-ID input parameter is still required to identify
...
by the graph query, and to typed empty inputs. `findCycles()` also reserves
`start` under the same rules. `shortestPath()` and `paths()` accept an original
column named `start` and preserve it; they each have only one requested start
and do not generate a starting-node column.
...
Downstream methods bind `gapParameter` through the existing queued values array
and cast it to `HUGEINT` in a one-row settings relation. The shared queue helper
materializes preceding work, runs the invalid-event existence check, and only
then permits the graph SELECT to execute. This barrier makes validation
observable even when a later `LIMIT 0`, false filter, unknown start, or missing
route could let DuckDB discard the graph computation. Named outputs validate the
copied input snapshot before replacing it with graph results. Graph SQL still
projects `eventSelections()` once, filters with `eventValidity()`, and calls
`transition()` with the settings relation's gap expression. The guarded
predicate prevents null or infinite values from reaching epoch conversion. The
helper owns timestamp comparison; each method keeps its traversal, pruning,
route, or component algorithm explicit.
...
events are excluded by this test-only evaluator so it can model valid subsets
independently; public methods reject such input before traversal. Shared
fixtures cover a standalone flight, an impossible and exact-gap transfer, and an
equal-time return cycle.
...
## Cycles from requested starting nodes

`findCycles(sourceColumn, targetColumn, edgeId, startNodes, options)` requires
one starting node ID or an array, using the same ID types and normalization as
`reachable()` and `distances()`. Empty arrays and duplicate starting IDs throw
an error; unknown IDs produce no cycles. Both static and chronological searches
restrict their initial events to the requested starts before recursive
enumeration. There is no length or output cap, so even one start can produce a
large number of cycles.

Each cycle starts and ends at its requested node. Different requested starts
produce separate results even when their cycles use the same connections in
rotated order. The `start` output column identifies the requested search root,
including incoming searches. Its type is the common source and target ID type.
`pathId` restarts at zero within each root, so the pair `(start, pathId)`
identifies a cycle. Static searches do not require the starting node to be the
smallest node. With `direction: "both"`, reverse sequences are deduplicated
within each start by retaining the lexicographically smaller edge-key sequence.
Cycles are numbered by edge-key sequence within each start; rows are sorted by
starting node key, then `pathId`, then `step`. Numeric keys use native numeric
order; string keys use binary bytes, independently of collations. Input row
order and the order of requested starts do not affect numbering. Parallel
connections with distinct IDs remain distinct.

Chronological searches retain the actual last event, visited nodes, and used
physical events. A candidate must pass the shared transition predicate. Reaching
the requested starting node closes the cycle immediately: the final event is
compared with its predecessor, but no transition is imposed from the final event
back to the first. A self-connection is therefore a one-event cycle regardless
of the configured gap. Returned cycles are never rotated to a different start.

Time rules apply separately from each requested start. For physical events B → C
at 09:00, C → A at 10:00, and A → B at 11:00, an outgoing search from
`["A", "B"]` returns only B → C → A → B. An incoming search from B follows A →
B, C → A, and B → C in 11:00, 10:00, 09:00 order. Steps follow search order, but
original endpoint values remain unchanged. With equal timestamps and non-strict
ordering, multiple requested starts may each yield a valid cycle; each is
returned separately.
...
can overlap, so one node can produce several `componentId`/`node` rows. They are
...
| `distances()` items 1–4: hop/weight optimum, costlier early state, justified dominance without route enumeration, cost units                                                                                                      | “Distance state and cost dominance”; `distances chronological state keeps cheap late and costlier early arrivals`, `distances chronological traversal handles first events, returns, and equal-time zero costs`, `distances chronological SQL uses finite keyed event costs without route enumeration`, and its generated evaluator comparison. Public JSDoc defines distance as hop count or summed connection weight, not elapsed time.                                                                                                                                                                                                                                                        |
...
| `findCycles()` items 1–4: feasible returns, non-smallest-node start, requested-start identity without closing gap, loops/equal times/directions/no reuse                                                                          | “Cycles from requested starting nodes”; `findCycles starts chronological cycles at a feasible event in both directions`, `findCycles applies strict, non-strict, gap, and self-loop chronology`, `findCycles preserves temporal parallel identities and deterministic cycle IDs`, and `findCycles matches an independent per-root chronological oracle`.                                                                                                                                                                                                                                                                                                                                         |
...
| Acceptance 1: unchanged static memberships, IDs, row order, and graph rules                                                                                                                                                       | Static calls retain their graph rules except for the approved required `findCycles()` starting nodes: cycles are now returned separately per requested start, with a generated `start` column and numbering by connection sequence restarting within each start. The approved output changes are `componentId` before `node` for `connectedComponents()`, and preservation of original columns after four generated columns for `shortestPath()` and `paths()` or five for `findCycles()`. Route endpoints now retain original physical values. Method tests cover revised schemas, empty results, and queued operations. Other method schemas, including `topologicalSort()`, remain unchanged. |

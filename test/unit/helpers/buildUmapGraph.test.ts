...
for (const reference of fixture.cases) {
  for (const search of ["exact", "hnsw"] as const) {
    Deno.test(`UMAP ${reference.metric}/${search} graph agrees with Python UMAP, including duplicates`, async () => {
      ...
      try {
        ...
        const graph = await buildUmapGraph(
          ...
          { metric: reference.metric, search },
          {
            rows: '"vectors"',
            knn: '"knn"',
            search: '"search"',
        ...
        const scales = (await c.runAndReadAll(
          "SELECT rho,sigma FROM scales ORDER BY source",
        ))
          .getRowsJS();
